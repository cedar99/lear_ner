...
def evaluate(args, model, processor, input_file, output=False, output_eval_info=False, data_type='dev'):
    ...
    if args.model_name == 'SERS':
        ...
        batch_label_token_ids, batch_label_token_type_ids, batch_label_input_mask, label_lexicon_embeddings_list  = processor.get_label_data(args.device)
        ...
        with torch.no_grad():
            ...
            if args.model_name == 'SERS':
                results = model(data, batch_label_token_ids,
                                batch_label_token_type_ids, batch_label_input_mask, label_lexicon_embs=label_lexicon_embeddings_list, return_score=args.dump_result, mode='inference', return_bert_attention=args.visualizate_bert)
...
def train(args, model, tokenizer, processor):
    ...
    ##
    # logger.info(f"train dataset: {type(train_dataset)}")
    # logger.info(f"train dataset: {type(train_dataset[0])}")
    # logger.info(f"train dataset: {train_dataset[0].soft_lexicon_embs.shape}")
    ...
    if args.model_name == 'SERS':
        ...
        batch_label_token_ids, batch_label_token_type_ids, batch_label_input_mask, label_lexicon_embeddings_list = processor.get_label_data(args.device)
        # logger.info("==================================")
        # logger.info(f"label lexcions shape: {label_lexicon_embeddings_list.shape}")
        # logger.info(f"batch label token ids shape: {batch_label_token_ids.shape}")
        global random_label_emb
        if args.use_random_label_emb and random_label_emb is None:
            random_label_emb = torch.rand(size=batch_label_token_ids.shape + (1024,))
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for cur_step, data in enumerate(train_bar):
                ...
                # logger.info(f"data  soft lexicon shape: {data['soft_lexicon_embs'].shape}")
