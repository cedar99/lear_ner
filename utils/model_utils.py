...
class Classifier_Layer(nn.Module):
    ...
    def forward(self, input):
        ...
        #                         self.class_num)
        # logger.info(f"weight shape: {self.weight.shape}")
...
class Label_Fusion_Layer_for_Token(nn.Module):
    ...
    def forward(self, token_embs, label_embs, input_mask, label_input_mask=None, return_scores=False, use_attn=False, do_add=False, average_pooling=False):
        ...
        # logger.info("Label_Fusion_Layer_for_Token: ")
        # logger.info(f"token embs shape: {token_embs.shape}")
        # logger.info(f"label embs shape: {label_embs.shape}")
    ...
    def get_fused_feature_with_attn(self, token_feature, label_feature, input_mask, label_input_mask, return_scores=False):
        ...
        """
        # print('attn')
        # logger.info("get_fused_feature_with_attn: ")
        # logger.info(f"token feature shape: {token_feature.shape}")
        # logger.info(f"label feature shape: {label_feature.shape}")
        
        batch_size, context_seq_len = token_feature.shape[:2]
        class_num = label_feature.shape[0]

        token_feature_fc = self.fc_1(token_feature)
        label_feature = self.fc_2(label_feature)

        # logger.info(f"token_feature_fc shape: {token_feature_fc.shape}")
        # logger.info(f"label_feature_fc shape: {label_feature.shape}")

        # [hidden_dim, class_num, label_seq_len]
        label_feature_t = label_feature.permute(
            2, 0, 1).view(self.hidden_size, -1)         # [hidden_dim, class_num*seq_len]
        # label_feature_t_norm = nn.functional.normalize(
        #     label_feature_t, p=2, dim=0)
        ...
        # print("toke_feature_fc: ", token_feature_fc.shape)
        # print("label_feature_t: ", label_feature_t.shape)
        scores = torch.matmul(token_feature_fc, label_feature_t).view(
            batch_size, context_seq_len, class_num, -1)  # dot, [batch_size, context_seq_len, class_num*label_seq_len] = [batch_size, context_seq_len, hidden_dim] * [hidden_dim, class_num*label_seq_len]

        # scores = scores / math.sqrt(self.hidden_size)
        # print("label num: ", self.label_num)
        # print(scores.shape)
        # print(label_input_mask.shape)

        extended_mask = label_input_mask[None, None, :, :]  # [1, 1, class_num, labe_seq_len]
        ...
        # print(token_feature_fc.shape)
        # print(weighted_label_feature_sum.shape)
        # print("=================================")
        ...
        # logger.info(f"fused_feature shape: {fused_feature.shape}")
        return output
