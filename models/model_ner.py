...
class LEARNer4Flat(nn.Module):
    def __init__(self, args):
        ...
        self.add_soft_lexicon = args.add_soft_lexicon
        # if add soft lexicon embs, we need to modify it, new hidden_size = hidden_size + lexicon_embs_size
        if self.add_soft_lexicon:
            self.hidden_size = self.encoder_config['hidden_size'] + 1200
        else:
            self.hidden_size = self.encoder_config['hidden_size']
        ...
        self.label_fusing_layer = model_utils.Label_Fusion_Layer_for_Token(
            self.hidden_size, self.label_num, 200 if self.use_label_embedding else self.hidden_size)
    ...
    def forward(self, data, label_token_ids, label_token_type_ids, label_input_mask, label_lexicon_embs=None, add_label_info=True, return_score=False, mode='train', return_bert_attention=False):
        ...
        ###
        # logger.info(f"data shape: {encoded_text.shape}")
        # logger.info(f"label shape: {label_embs.shape}")

        # soft_lexicon_embs = data["soft_lexicon_embs"]
        # logger.info(f"data lexicon shape: {soft_lexicon_embs.shape}")
        # logger.info(f"label lexicon shape: {label_lexicon_embs.shape}")

        # add soft lexicon for data and label
        if self.add_soft_lexicon:
            encoded_text = torch.cat((encoded_text, data['soft_lexicon_embs']), -1)
            label_embs = torch.cat((label_embs, label_lexicon_embs), -1)
        # logger.info(f"encode_text shape: {encoded_text.shape}")
        # logger.info(f"label_embs shape: {label_embs.shape}")
